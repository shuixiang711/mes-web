<template>
  <div class="app-container">
    <el-table v-loading="loading" :data="transferlineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="物资名称" width="150px" align="center" prop="itemName" />
      ...
      <el-table-column label="移出仓库" align="center" prop="warehouseName" />
      <el-table-column label="移出库区" width="120px" align="center" prop="locationName" />
      <el-table-column label="移出库位" align="center" prop="areaName" />
...
<script>
import { listTransferdetail, getTransferdetail, delTransferdetail, addTransferdetail, updateTransferdetail } from "@/api/mes/wm/transferdetail";
import { listWithDetail } from "@/api/mes/wm/transferline";
...
export default {
  ...
  methods: {
    ...
    handleAdd(row) {
      this.reset();
      this.form.transferId = row.transferId
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除转移调拨单明细编号为"' + detailIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('wm/transferdetail/export', {
        ...this.queryParams
