...
import {getLocationName} from "../../../../api/mes/wm/area";
export default {
  name: "Area",
  dicts: ['sys_yes_no'],
  mixins: [hiprintMixin],
  ...
  methods: {
    ...
    async handleHiPrint(row) {
      let printData = row
      let printTemplate
      getLocationName(row).then(res => {
        printData.locationName = res.data.locationName
        printData.barcodeContent = res.data.barcodeContent
      })
      // 获取打印模板
      let templateStatus = true
      await getByTemplateType("AREA").then(res => {
        templateStatus = true
