  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['wm:miscrecptdetail:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['wm:miscrecptdetail:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['wm:miscrecptdetail:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['wm:miscrecptdetail:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="miscrecptdetailList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="明细ID" align="center" prop="detailId" />
      <el-table-column label="行ID" align="center" prop="lineId" />
      <el-table-column label="杂项入库单ID" align="center" prop="recptId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="行ID" prop="lineId">
          <el-input v-model="form.lineId" placeholder="请输入行ID" />
        </el-form-item>
        <el-form-item label="杂项入库单ID" prop="recptId">
          <el-input v-model="form.recptId" placeholder="请输入杂项入库单ID" />
        </el-form-item>
        <el-form-item label="产品物料ID" prop="itemId">
...
<script>
import { listMiscrecptdetail, getMiscrecptdetail, delMiscrecptdetail, addMiscrecptdetail, updateMiscrecptdetail } from "@/api/mes/wm/miscrecptdetail";

export default {
  name: "Miscrecptdetail",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMiscrecptdetail(this.queryParams).then(response => {
        this.miscrecptdetailList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加杂项入库单明细";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const detailId = row.detailId || this.ids
      getMiscrecptdetail(detailId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改杂项入库单明细";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.detailId != null) {
            updateMiscrecptdetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMiscrecptdetail(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除杂项入库单明细编号为"' + detailIds + '"的数据项？').then(function() {
        return delMiscrecptdetail(detailIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('wm/miscrecptdetail/export', {
        ...this.queryParams
      }, `miscrecptdetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
