  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['wm:miscissuedetail:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['wm:miscissuedetail:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['wm:miscissuedetail:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['wm:miscissuedetail:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="miscissuedetailList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="明细ID" align="center" prop="detailId" />
      <el-table-column label="行ID" align="center" prop="lineId" />
      <el-table-column label="杂项出库单ID" align="center" prop="issueId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="杂项出库单ID" prop="issueId">
          <el-input v-model="form.issueId" placeholder="请输入杂项出库单ID" />
        </el-form-item>
        <el-form-item label="产品物料ID" prop="itemId">
...
<script>
import { listMiscissuedetail, getMiscissuedetail, delMiscissuedetail, addMiscissuedetail, updateMiscissuedetail } from "@/api/mes/wm/miscissuedetail";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMiscissuedetail(this.queryParams).then(response => {
        this.miscissuedetailList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加杂项出库单明细";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const detailId = row.detailId || this.ids
      getMiscissuedetail(detailId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改杂项出库单明细";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.detailId != null) {
            updateMiscissuedetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMiscissuedetail(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除杂项出库单明细编号为"' + detailIds + '"的数据项？').then(function() {
        return delMiscissuedetail(detailIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('wm/miscissuedetail/export', {
        ...this.queryParams
      }, `miscissuedetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
