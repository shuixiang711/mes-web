...
<script>
import { getOutsourceissuedetail, delOutsourceissuedetail, addOutsourceissuedetail, updateOutsourceissuedetail } from "@/api/mes/wm/outsourceissuedetail";
import { listWithDetail } from "@/api/mes/wm/outsourceissueline";
...
export default {
  ...
  methods: {
    ...
    handleAdd(row) {
      ...
      this.open = true;
      this.title = "添加外协领料单明细";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const detailId = row.detailId || this.ids
      getOutsourceissuedetail(detailId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改外协领料单明细";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.detailId != null) {
            updateOutsourceissuedetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getListWithDetail();
            });
          } else {
            addOutsourceissuedetail(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除外协领料单明细编号为"' + detailIds + '"的数据项？').then(function() {
        return delOutsourceissuedetail(detailIds);
      }).then(() => {
        this.getListWithDetail();
