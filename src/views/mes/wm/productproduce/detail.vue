<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="明细ID" prop="lineId">
        <el-input
      ...
      <el-form-item label="产出数量" prop="quantity">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['wm:productproducedetail:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['wm:productproducedetail:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['wm:productproducedetail:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['wm:productproducedetail:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="productproducedetailList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="行ID" align="center" prop="detailId" />
      <el-table-column label="明细ID" align="center" prop="lineId" />
      ...
      <el-table-column label="产出数量" align="center" prop="quantity" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="明细ID" prop="lineId">
          <el-input v-model="form.lineId" placeholder="请输入明细ID" />
        </el-form-item>
        <el-form-item label="产出记录ID" prop="recordId">
        ...
        <el-form-item label="产出数量" prop="quantity">
          <el-input v-model="form.quantity" placeholder="请输入产出数量" />
        </el-form-item>
        <el-form-item label="批次ID" prop="batchId">
...
<script>
import { listProductproducedetail, getProductproducedetail, delProductproducedetail, addProductproducedetail, updateProductproducedetail } from "@/api/mes/wm/productproducedetail";

export default {
  name: "Productproducedetail",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listProductproducedetail(this.queryParams).then(response => {
        this.productproducedetailList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加产品产出记录明细";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const detailId = row.detailId || this.ids
      getProductproducedetail(detailId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改产品产出记录明细";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.detailId != null) {
            updateProductproducedetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addProductproducedetail(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除产品产出记录明细编号为"' + detailIds + '"的数据项？').then(function() {
        return delProductproducedetail(detailIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('wm/productproducedetail/export', {
        ...this.queryParams
      }, `productproducedetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
