<template>
  <div class="app-container">
    <el-table v-loading="loading" :data="rtissuelineList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="是否检验" width="100px" align="center" prop="iqcCheck" >
        <template slot-scope="scope">
...
<script>
import { getOutsourcerecptdetail, delOutsourcerecptdetail, addOutsourcerecptdetail, updateOutsourcerecptdetail } from "@/api/mes/wm/outsourcerecptdetail";
import { listWithDetail } from "@/api/mes/wm/outsourcerecptline";
import {getTreeList} from "@/api/mes/wm/warehouse"
import printLabel from "@/components/printerLabel/index.vue"
export default {
  name: "OutsourcerecptDetail",
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getOutsourcerecptdetail(detailId).then(response => {
        that.form = response.data;
        that.$set(that.warehouseInfo,0,response.data.warehouseId);
        that.$set(that.warehouseInfo,1,response.data.locationId);
        that.$set(that.warehouseInfo,2,response.data.areaId);
        that.open = true;
        that.title = "修改外协入库单明细";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.detailId != null) {
            updateOutsourcerecptdetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOutsourcerecptdetail(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除外协入库单明细编号为"' + detailIds + '"的数据项？').then(function() {
